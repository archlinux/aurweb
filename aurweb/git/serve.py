...
def pkgreq_open_adoption(pkgbase_id, pkgbase, userid):
    conn = aurweb.db.Connection()

    cur = conn.execute("SELECT ID FROM RequestTypes WHERE Name = ?", ["adoption"])
    row = cur.fetchone()
    if not row:
        conn.close()
        die("The adoption request type is missing; contact the AUR maintainers.")
    reqtype_id = row[0]

    now = int(time.time())
    cur = conn.execute(
        "INSERT INTO PackageRequests (ReqTypeID, PackageBaseID, "
        + "PackageBaseName, UsersID, Comments, ClosureComment, "
        + "RequestTS, Status) VALUES (?, ?, ?, ?, ?, ?, ?, 0)",
        [
            reqtype_id,
            pkgbase_id,
            pkgbase,
            userid,
            "Autogenerated by aurweb (SSH).",
            "",
            now,
        ],
    )
