...
@db.retry_deadlock
def _retry_request_adoption(request: Request, pkgbase: PackageBase) -> PackageRequest:
    with db.begin():
        pkgreq = db.create(
            PackageRequest,
            ReqTypeID=ADOPTION_ID,
            User=request.user,
            RequestTS=time.utcnow(),
            PackageBase=pkgbase,
...
def pkgbase_adopt_instance(request: Request, pkgbase: PackageBase) -> None:
    ...
    pkgreq = _retry_request_adoption(request, pkgbase)
    notify.RequestOpenNotification(
        request.user.ID, pkgreq.ID, ADOPTION, pkgbase.ID
    ).send()
